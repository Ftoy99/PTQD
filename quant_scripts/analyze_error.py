...
def get_t_residualerror_std_dict(t_tensor : Tensor, data_tensor, error_tensor):
    '''
        need kt first for calculating residual error
    '''
    kt_dict = np.load('correct_data/imagenet_20steps_w{}a{}/kt.npy'.format(n_bits_w, n_bits_a),
    ...
    print(t_std_dict)
    np.save("correct_data/imagenet_20steps_w{}a{}/t_std_dict.npy".format(n_bits_w, n_bits_a), t_std_dict,
            allow_pickle=True)


if __name__ == '__main__':
    data_error_t_list = torch.load('data_error_t_w{}a{}_scale3.0_eta0.0_step20.pth'.format(n_bits_w, n_bits_a),
                                   map_location='cpu')  ## replace error file here
    data_list = []
    error_list = []
    t_list = []
